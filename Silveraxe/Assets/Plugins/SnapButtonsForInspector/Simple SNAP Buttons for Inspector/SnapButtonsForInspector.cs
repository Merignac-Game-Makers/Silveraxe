...
namespace EModules.Snap
{
	...
	class Snap
	{
		...
		static PRF[] POS_PREFKEYS = { _PRF("MoveSnapX", .25f), _PRF("MoveSnapY", .25f), _PRF("MoveSnapZ", .25f) };
		...
		private static void SceneView_PLACEONSURFACE(SceneView sceneView) {
			...
			if (Event.current.type == EventType.MouseDown && (Selection.GetTransforms(SelectionMode.TopLevel | SelectionMode.OnlyUserModifiable).Length > 0)) {   /* if ( PLACE_ON_SURFACE_ALIGNBYMOUSE )
		...
		void DO_SNAP_BUTTON(Rect r, VectorPref pref) {
			if (GUI.Button(r, snapContent, buttonStyle)) {
				if (Event.current.button == 0) pref.ENABLE.Set(!pref.ENABLE);
					...
					menu.AddSeparator("");
					menu.AddItem(new GUIContent("Open Unity Snap Settings"), false, () => EditorApplication.ExecuteMenuItem("Edit/Grid and Snap Settings..."));
